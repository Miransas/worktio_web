...
// flows.length === 0 
const TEMPLATES = [
  { name: "Gmail Otomasyonu", description: "Gelen mailleri AI ile kategorize et", nodes: 3 },
  { name: "AI İçerik Üretici", description: "Gemini ile sosyal medya içeriği", nodes: 2 },
  { name: "HTTP → AI → Mail", description: "API çek, AI işle, mail gönder", nodes: 3 },
];
...
export default function FlowListPage() {
  ...
  return (
    <div className="p-8 text-zinc-100">
      ...
      <div className="mt-12">
        <h2 className="text-sm font-semibold text-zinc-400 mb-4 uppercase tracking-wider">Şablonlar</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {TEMPLATES.map(t => (
            <div
              key={t.name}
              className="bg-zinc-900 border border-zinc-800 hover:border-purple-500/50 rounded-xl p-5 cursor-pointer transition-all group"
              onClick={createFlow}
            >
              <div className="text-sm font-semibold text-zinc-200 mb-1 group-hover:text-purple-400 transition-colors">{t.name}</div>
              <div className="text-xs text-zinc-500">{t.description}</div>
              <div className="text-xs text-zinc-600 mt-3">{t.nodes} node</div>
            </div>
          ))}
        </div>
      </div>
