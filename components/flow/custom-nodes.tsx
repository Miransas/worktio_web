...
import { Zap, Brain, Globe, GitBranch, Code2, Mail, Send } from "lucide-react";
...
const NODE_CONFIGS: Record<string, NodeConfig> = {
  ...
  ai:        { icon: <Brain size={16} />,   label: "AI",           color: "text-purple-400",  bg: "bg-purple-500/10",  border: "border-purple-500/30",  iconBg: "bg-purple-500/20" },
  ...
  telegram:  { icon: <Send size={16} />,    label: "Telegram",     color: "text-sky-400",     bg: "bg-sky-500/10",     border: "border-sky-500/30",     iconBg: "bg-sky-500/20" },
};
...
function BaseNode({ data, type, selected }: NodeProps & { type: string }) {
  ...
  const subtitle = (data?.url as string) || (data?.prompt as string) || (data?.to as string) || (data?.chatId as string) || null;
  ...
  return (
    ...
    `}>
      ...
      <div className="px-3 py-2.5">
        ...
        {subtitle && <div className="text-xs text-zinc-500 truncate max-w-[180px] mt-0.5">{subtitle}</div>}
      </div>
...
export const TelegramNode  = (p: NodeProps) => <BaseNode {...p} type="telegram" />;
